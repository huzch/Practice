...
public:
	...
	void Assign(char var, DATATYPE x)
	{
		_Assign(_root, var, x);
	}
...
protected:
	...
	void _Assign(Node* root, char var, DATATYPE x)
	{
		if (root == nullptr)
		{
			return;
	...
	DATATYPE _Value(Node* root)
	{
		...
		DATATYPE leftVal = _Value(root->_left);
		DATATYPE rightVal = _Value(root->_right);
	...
	DATATYPE Calc(const string& func, DATATYPE x)
	{
		if (func == "sin")
		{
			return sin(x);
	...
	DATATYPE Calc(char op, DATATYPE x, DATATYPE y)
	{
		if (op == '+')
		{
			return x + y;
	...
	void ToNum(Node* root, DATATYPE x)
	{
		root->_num = x;
		root->_isCh = false;
		root->_isOp = false;
	...
	void _MergeConst(Node* root)
	{
		...
		if (root->_isFunc)
		{
			...
			if (IsCon(x))
			{
				DATATYPE ret = Calc(root->_func, x->_num);
				ToNum(root, ret);
			}
		}
		else if (root->_isOp)
		{
			...
			if (IsCon(x) && IsCon(y))
			{
				DATATYPE ret = Calc(root->_ch, x->_num, y->_num);
				ToNum(root, ret);
			}
	...
	void ToExpr(Node* root, Node* e)
	{
		root->_ch = e->_ch;
		root->_func = e->_func;
		root->_isCh = e->_isCh;
		root->_isOp = e->_isOp;
		root->_isFunc = e->_isFunc;
		root->_left = e->_left;
		root->_right = e->_right;
	}

	string _GetInfix(Node* root) const
	{
		if (root == nullptr)
		{
			return "";
		...
		{
			ret += format("{:.2f}", root->_num);
		}

		ret += _GetInfix(root->_right);
		if (root->_isOp)
		{
			ret += ")";
		}
